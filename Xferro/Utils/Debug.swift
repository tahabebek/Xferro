...
enum Signpost {
  static let logger = OSLog(subsystem: "com.xferro", category: .pointsOfInterest)
  static let debugLogger = Logger(subsystem: "com.xferro", category: "signpost")
  ...
  static func interval<T>(_ code: Interval, call: () throws -> T) rethrows -> T {
  ...
  static func interval<T>(_ code: Interval, call: () async throws -> T) async rethrows -> T {
    let id = OSSignpostID(log: Signpost.logger)
