...
final class PrefsWindowController: NSWindowController {
        ...
        func makeController(size: CGSize) -> NSViewController {
            ...
            case .general:
                return NSHostingController(
                    rootView: GeneralPrefsPane(defaults: UserDefaults.standard, config: GitConfig.default!)
                        .padding().fixedSize()
                )
                .sizedToFit(in: size)
            case .accounts:
    ...
    override func windowDidLoad() {
        ...
        for tab in Tab.allCases {
            ...
            tabItem.image = .init(systemSymbolName: tab.imageName, accessibilityDescription: tab.rawValue)
