...
struct PreviewsPrefsPane: View {
    ...
    init(defaults: UserDefaults) {
        _fontName = .init(
            wrappedValue: defaults.fontName,
            PreferenceKeys.fontName.key,
            store: defaults
        )
        _fontSize = .init(
            wrappedValue: defaults.fontSize,
            PreferenceKeys.fontSize.key,
            store: defaults
        )
        _whitespace = .init(
            wrappedValue: defaults.whitespace,
            PreferenceKeys.diffWhitespace.key,
            store: defaults
        )
        _wrapping = .init(
            wrappedValue: defaults.wrapping,
            PreferenceKeys.wrapping.key,
            store: defaults
        )
        _tabWidth = .init(
            wrappedValue: defaults.tabWidth,
            PreferenceKeys.tabWidth.key,
            store: defaults
        )
        _contextLines = .init(
            wrappedValue: defaults.contextLines,
