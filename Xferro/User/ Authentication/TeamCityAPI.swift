...
protocol BuildStatusService: AnyObject {
  func displayName(longLocalBranchName: String, buildType: String) -> String?
...
final class TeamCityAPI: BasicAuthService, ServiceAPI, BuildStatusService {
  ...
  required init?(account: Account, password: String) {
    guard var fullBaseURL = URLComponents(url: account.location, resolvingAgainstBaseURL: false)
    else { return nil }
    
    fullBaseURL.path = TeamCityAPI.rootPath
    
    guard let location = fullBaseURL.url else { return nil }
    var account = account
    ...
    configure(description: "xml") {
      $0.pipeline[.parsing].add(XMLResponseTransformer(), contentTypes: [ "*/xml" ])
    }
  }
  ...
  @MainActor
    func enumerateBuildStatus(
        longLocalBranchName: String,
        buildType: String,
        processor: @escaping ([String: String]) -> Void
    ) {
    let statusResource = buildStatus(longLocalBranchName, buildType: buildType)
  ...
  /// Use the branchSpecs to determine the display name for the given build type
    ...
    let vcsRoots = vcsRootsForBuildType(buildType)
        let displayNames = vcsRoots.compactMap {
            vcsBranchSpecs[$0]?.match(branch: longLocalBranchName)
        }

    return displayNames.reduce(nil) { shortest, name -> String? in
  ...
  func buildTypesForRemote(_ remoteURLString: String) -> [String] {
    ...
    let result = buildTypeURLs.keys.filter { key in
      buildTypeURLs[key].map { urls in
  ...
  func vcsRootsForBuildType(_ buildType: String) -> [String] {
    ...
    guard let urls = buildTypeURLs[buildType] else { return [] }
    let result = vcsRootMap.compactMap { vcsRoot, rootURL in
  ...
  func getBuildTypes() async throws {
    ...
    guard let xml = data.content as? XMLDocument else {
      throw Error.parseFailure(.buildTypes)
    }
    guard let hrefs = xml.rootElement()?.childrenAttributes(Build.Attribute.href) else {
  ...
  func parseBuildType(_ xml: XMLDocument) throws {
    guard let buildType = xml.children?.first as? XMLElement else {
      throw Error.firstBuildType
    }

    let name = buildType.attribute(forName: "name")?.stringValue ?? "❎"

    guard let rootEntries = buildType.elements(forName: "vcs-root-entries").first else {
      throw Error.rootEntries
    }
    guard let buildTypeID = buildType.attribute(forName: "id")?.stringValue else {
...
extension BuildStatusAccessor {
  ...
  func matchBuildStatusService(_ remoteName: String) -> (BuildStatusService, [String])? {
...
extension TeamCityAPI: RemoteService {
  func match(remote: Remote) -> Bool {
    guard let urlString = remote.url?.absoluteString else { return false }
