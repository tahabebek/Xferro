...
final class BitbucketServerAPI: BasicAuthService, ServiceAPI {
    ...
    struct PullRequest: Xferro.PullRequest {
...
#warning("Implement logic to determine who can merge")
                return []
            default:
                return []
            }
    ...
    required init?(account: Account, password: String) {
        ...
        super.init(account: account, password: password, authenticationPath: "users/\(account.user)")
    ...
    override func didAuthenticate(responseResource: Resource) {
        Task.detached {
            ...
            guard let user = content as? BitbucketServer.User else {
    ...
    func setStatus(
        ...
        let href = """
          ...
          """
        let resource = self.resource(absoluteURL: URL(string: href, relativeTo: baseURL))
...
extension BitbucketServerAPI: PullRequestService {
    ...
    func getPullRequests() async -> [Xferro.PullRequest] {
        do {
    ...
    func update(
        ...
        guard let userSlug = user?.slug else { return }

        let resource = self.resource(pullRequestPath(request) + "participants/\(userSlug)")
        let data: [String: Any] = [
            "user": ["slug": userSlug],
            "approved": approved,
            "status": status.rawValue
    ...
    func approve(request: Xferro.PullRequest) async throws {
        if let request = request as? PullRequest {
            try await update(request: request, approved: true, status: .approved)
        }
    }

    func unapprove(request: Xferro.PullRequest) async throws {
        if let request = request as? PullRequest {
            try await update(request: request, approved: false, status: .unapproved)
        }
    }

    func needsWork(request: Xferro.PullRequest) async throws {
        if let request = request as? PullRequest {
            try await update(request: request, approved: false, status: .needsWork)
        }
    }

    func merge(request: Xferro.PullRequest) async throws {
        guard let request = request as? PullRequest
        else { return }
        try await self.resource(pullRequestPath(request) + "merge").request(.post).complete()
#warning("Is there anything else we need to do after merging a pull request?")
    }
}
