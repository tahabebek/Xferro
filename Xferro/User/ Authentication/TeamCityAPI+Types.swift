...
extension TeamCityAPI {
    public struct Build: Sendable {
        ...
        init?(element buildElement: XMLElement) {
            guard buildElement.name == "build" else { return nil }
        ...
        init?(xml: XMLDocument) {
            guard let build = xml.rootElement() else { return nil }
...
public class BranchSpec {
    ...
    struct Rule {
        ...
        init?(content: String) {
            let prefixEndIndex = content.index(content.startIndex, offsetBy: 2)
        ...
        func match(branch: String) -> String? {
            ...
            guard let match = regex.firstMatch(in: branch, options: .anchored, range: stringRange)
            else { return nil }
    ...
    class func defaultSpec() -> BranchSpec {
        BranchSpec(ruleStrings: ["+:refs/heads/*"])!
    }
