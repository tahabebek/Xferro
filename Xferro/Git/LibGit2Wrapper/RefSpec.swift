...
struct GitRefSpec: RefSpec {
    ...
    var direction: RemoteConnectionDirection {
        RemoteConnectionDirection(gitDirection: git_refspec_direction(refSpec))
    }
    ...
    func sourceMatches(refName: String) -> Bool {
        git_refspec_src_matches(refSpec, refName) == 1
    }

    func destinationMatches(refName: String) -> Bool {
        git_refspec_dst_matches(refSpec, refName) == 1
    }
