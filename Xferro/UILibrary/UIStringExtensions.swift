...
extension NSMenu {
    @discardableResult
    func addItem(
        withTitleString title: UIString,
        action: Selector?,
        keyEquivalent: String
    ) -> NSMenuItem {
        addItem(
            withTitle: title.rawValue,
            action: action,
...
extension NSMenuItem {
    ...
    convenience init(
        titleString: UIString,
        action: Selector?,
        keyEquivalent: String
    ) {
        self.init(
            title: titleString.rawValue,
            action: action,
            keyEquivalent: keyEquivalent
        )
    }
...
extension NSSegmentedControl {
    convenience init(
        labelStrings: [UIString],
        trackingMode: NSSegmentedControl.SwitchTracking,
        target: AnyObject,
        action: Selector
    ) {
        self.init(
            labels: labelStrings.map { $0.rawValue },
